...
// Wire, sdaPin(-1), sclPin(-1), i2cAddr(0x4B), clock(100kHz), intPin(2), rstPin(4)
BnoI2CBus bus4B(Wire, -1, -1, 0x4B, 100000, 2, 4);
BNO08x_7Semi imu4B(bus4B);

BnoI2CBus bus4A(Wire, -1, -1, 0x4A, 100000, 2, 4);
...
void setup() {
  ...
  if (imu4B.begin()) {
    ...
    delay(100);
    active_imu->enableReport(GAME_ROTATION_VECTOR, 20); // 20ms = 50 Hz
  } else if (imu4A.begin()) {
    active_imu = &imu4A;
