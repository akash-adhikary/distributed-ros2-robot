...
debug_sketch = """cat << 'SKETCH' > ~/BnoTest/BnoTest.ino
...
void setup() {
  ...
  // Send Set Feature Command: Enable Rotation Vector (0x05) at 50Hz (20000 us)
  uint8_t cmd[21] = {0};
  cmd[0] = 0xFD; // Set Feature Command
  cmd[1] = 0x05; // Rotation Vector
  cmd[5] = 0x20; cmd[6] = 0x4E; // 20000 us
  send_shtp(2, cmd, 21);
}
  ...
  if (i2c_write_byte((BNO_ADDR << 1) | 1)) {
    ...
    if (plen > 4 && plen < 128) {
      ...
      if (i2c_write_byte((BNO_ADDR << 1) | 1)) {
        ...
        // Check for rotation vector report anywhere in payload
...
for i in range(5):
    ...
    ch = call_mcu(s, 'imu/chan')
    l = call_mcu(s, 'imu/len')
    qr = call_mcu(s, 'imu/qr')
    qi = call_mcu(s, 'imu/qi')
    qj = call_mcu(s, 'imu/qj')
    qk = call_mcu(s, 'imu/qk')
    print(f'SHTP Sample {i}: count={c} chan={ch} len={l} quat=[{qr}, {qi}, {qj}, {qk}]')
    time.sleep(0.3)
s.close()
PYEOF
python3 ~/check_shtp_debug.py
"""
child.sendline(debug_sketch)
child.expect([r'SHTP Sample 4:'], timeout=90)
child.expect([r'\$ '], timeout=15)
