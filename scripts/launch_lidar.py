...
child.sendline("docker exec rplidar bash -c 'source /opt/ros/humble/setup.bash && source /ws/install/setup.bash && export ROS_DOMAIN_ID=42 && export CYCLONEDDS_URI=file:///ws/cyclonedds.xml && export RMW_IMPLEMENTATION=rmw_cyclonedds_cpp && timeout 10 ros2 topic hz /scan || true'")
