...
void setup() {
  ...
  if (imu4A.begin()) {
    active_imu = &imu4A;
    sensor_active = 1;
    active_imu->enableReport(GAME_ROTATION_VECTOR, 20000);
  } else if (imu4B.begin()) {
    active_imu = &imu4B;
    sensor_active = 2;
    active_imu->enableReport(GAME_ROTATION_VECTOR, 20000);
...
void loop() {
  if (active_imu != nullptr) {
    ...
    const State &st = active_imu->getState();
    if (st.hasGameQuat) {
...
sleep 2
echo "COMPILED_AND_UPLOADED"
"""
child.sendline(sketch_code)
child.expect([r'COMPILED_AND_UPLOADED'], timeout=120)
child.expect([r'\$ '], timeout=15)
