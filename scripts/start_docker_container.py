...
child.sendline("docker exec rplidar bash -c 'source /opt/ros/jazzy/setup.bash && source /ros_ws/install/setup.bash && export ROS_DOMAIN_ID=42 && export RMW_IMPLEMENTATION=rmw_cyclonedds_cpp && ros2 pkg list | grep rplidar'")
child.expect([r'arduino@blissy:\~\$ '], timeout=20)
