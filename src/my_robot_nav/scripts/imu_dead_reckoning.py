...
class ImuDeadReckoningNode(Node):
    def __init__(self):
        ...
        # VR gesture smoothing & drift protection
        self.quat_smooth_alpha = 0.35
        self.acc_smooth_alpha = 0.40
        self.acc_deadband = 0.20   # m/s^2
        self.gyro_deadband = 0.08  # rad/s
        self.max_angular_jump = 0.6
        self.max_acc_magnitude = 25.0
        self.vel_decay = 0.85      # Leaky velocity decay
        self.pos_decay = 0.99      # Smooth position centering
        ...
        self.get_logger().info("Jitter-Filtered 3D Motion Tracker active.")
    ...
    def imu_callback(self, msg):
        ...
        if not self.calibrated:
            ...
            if len(self.calib_samples) >= 25:
                self.gravity_mag = sum(self.calib_samples) / len(self.calib_samples)
                self.calibrated = True

        # 6. Omnidirectional Gravity Compensation in Body Frame
        ...
        # 7. Rotate linear motion into world coordinate frame
        ...
        # 9. Velocity Integration with Leaky Damping
        decay = math.pow(self.vel_decay, dt * 10.0)
        self.vel[0] = self.vel[0] * decay + a_world[0] * dt
        self.vel[1] = self.vel[1] * decay + a_world[1] * dt
        self.vel[2] = self.vel[2] * decay + a_world[2] * dt

        # 10. Position Integration with Smooth Spring Centering
        self.pos[0] = (self.pos[0] + self.vel[0] * dt) * self.pos_decay
        self.pos[1] = (self.pos[1] + self.vel[1] * dt) * self.pos_decay
        self.pos[2] = (self.pos[2] + self.vel[2] * dt) * self.pos_decay

        now_msg = now.to_msg()

        # 11. Dynamic TF Broadcast: odom -> base_link and base_link -> imu_link
